...
<div class="grid grid-flow-col auto-cols-auto">
	...
	<div class="flex flex-col mx-4 sm:mx-16">
		...
		<H2 id="tldr"><i class="font-serif">tl;dr</i></H2>
		<ul class="list-disc ms-16 text-sm">
			<li>
				This project explores the use of the NHiTS <Ref id="challu" /> neural network to forecast
				trajectories on the Lorenz Attractor. Given {`$2H$`} points of input, the model is trained
				to predict the next {`$H$`} points of the trajectory, where {`$H$`} is usually 100 and
				{`$dt \\approx 0.015$`}.
			</li>
			<li>
		...
		<P>
			...
			can vary dynamically, but the initial target value used by <Link
				href="https://github.com/williamgilpin/dysts">dysts</Link
			> is {`$\\approx 1.8\\mathrm{e}\\text{-}\\mathrm{4}$`}.
		</P><P
		...
		<p>
			However, there doesn't appear to be any strong evidence that continuing to reduce {`$dt$`}
			might lead to a fully stable model (<i>i.e.</i> one without an asymptotic error curve). And
			reducing it further will drastically increase the computational requirements of training,
			which are beyond my budget, so I must conclude this avenue of investigation here.
		</p>
		...
		<figure class="mb-2 self-center max-w-3xl">
			...
			/><FigCap>
				Comparing the distribution of an exponential function of the z coordinate for the
		...
		<P
			...
			that they avoid passing through this critical region around Z {`$\\approx 38.55$`}. The
			results of this study suggest that this alone might make the dynamics of the Lorenz
			Attractor practically predictable.
		</P>
		<P
			...
			local Z maximum <i>is</i> linearly separable between these two groups, assuming we could
		...
		<p class="my-4 text-sm font-serif bg-gray-100 rounded-sm border ring-1 ring-black p-2">
			...
			smaller {`$dt$`}
			parameter in the IVP solver. I tried values of {`$dt$`} in the range {`$[1.8\\mathrm{e}\\text{-}\\mathrm{4}, 1.8\\mathrm{e}\\text{-}\\mathrm{5}]$`}.
			In all cases, the IVP solver eventually produced a trajectory that violated the linear
			separability requirement for the local Z maximum, and at {`$1.8\\mathrm{e}\\text{-}\\mathrm{5}$`},
			the numerical stability of the solver appeared to break down.
		</p>
		<P
